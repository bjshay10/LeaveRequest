...
<cfif (cgi.https eq "off") and 
	...
	<cflocation url="https://www.mesa.k12.co.us/apps/LeaveRequest/Test/supervisornew.cfm" addtoken="no">
	<cfabort>
</cfif>
...
<cfelseif StepNum eq 1>
    ...
    <cfquery name="GetRequests" datasource="mesa_web">
        ...
        WHERE	(((supervisor = '#Session.email#') OR (supervisor = '#session.username#@d51schools.org')) AND (supviewed IS NULL) AND yearofrequest >= '2017-2018')
        or 
        (((supervisor2 = '#Session.email#') OR (supervisor2 = '#session.username#@d51schools.org')) AND (sup2viewed IS NULL) AND yearofrequest >= '2017-2018')
    ...
    <cfquery name="GetRequests" datasource="mesa_web">
    	SELECT *
        FROM	LeaveReq_tblRequest
        WHERE	(((supervisor = '#Session.email#') OR (supervisor = '#session.username#@d51schools.org')) AND (supviewed = 'Yes' or supviewed = 'No') AND yearofrequest >= '2017-2018')
        or 
        (((supervisor2 = '#Session.email#') OR (supervisor2 = '#session.username#@d51schools.org')) AND (sup2viewed = 'Yes' or supviewed = 'No') AND yearofrequest >= '2017-2018')
        ORDER BY DateEntered, userid
    </cfquery>
