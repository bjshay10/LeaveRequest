...
<!--- Check Username and Password --->
	<cfif isdefined ("submituser")>        
        ...
        <cfif getaccounts.recordcount eq 0>
            <cflocation url="supervisor.cfm?tryagain" addtoken="no">
        <cfelse>
...
<cfelseif StepNum eq 1>
    ...
    <cfquery name="GetRequests" datasource="mesa_web">
        ...
        WHERE	(((supervisor = '#Session.email#') OR (supervisor = '#session.username#@d51schools.org')) AND (supviewed IS NULL) )
        or 
        (((supervisor2 = '#Session.email#') OR (supervisor2 = '#session.username#@d51schools.org')) AND (sup2viewed IS NULL))
    </cfquery>
    ...
    <cfquery name="GetRequests" datasource="mesa_web">
    	SELECT *
        FROM	LeaveReq_tblRequest
        WHERE	(((supervisor = '#Session.email#') OR (supervisor = '#session.username#@d51schools.org')) AND (supviewed = 'Yes') )
        or 
        (((supervisor2 = '#Session.email#') OR (supervisor2 = '#session.username#@d51schools.org')) AND (sup2viewed = 'Yes'))
    </cfquery>
    <cfif #GetRequests.Supervisor# eq '#Session.email#'>
    	<cfset Session.SupNum = 1>
    <cfelseif #GetRequests.Supervisor# eq '#session.username#@d51schools.org'>
