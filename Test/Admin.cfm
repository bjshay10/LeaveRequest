...
<cfif (cgi.https eq "off") and 
	...
	<cflocation url="https://www.mesa.k12.co.us/apps/LeaveRequest/Test/Admin.cfm" addtoken="no">
	<cfabort>
</cfif>
...
<cfelseif StepNum eq 10>
    ...
    <cfquery name="GetRequests" datasource="mesa_web">
        ...
        WHERE	Approved IS NULL and yearofrequest = '2017-2018'
...
<cfelseif StepNum eq 11>
    ...
    <cfquery name="GetDaysApp" datasource="mesa_web">
        ...
        FROM	LeaveReq_tblRequest
        WHERE	userid = '#Session.userid#' and yearofrequest = '2017-2018' and Approved = 'A'
    </cfquery>
    <table border="1" width="100%">
    	<tr>
...
Your leave request for the following day(s): #LSDateFormat(GetReqInfo.dtFrom,'mm/dd/yyyy')# - #LSDateFormat(GetReqInfo.dtTo,'mm/dd/yyyy')#, <cfif #GetReqInfo.approved# eq 'A'>has been approved and will be paid for, pending receipt of proper documentation.   If you have not already done so, please submit documentation via email or interdepartmental mail to the Human Resources Office, attention Julie Bitner, prior to the last working day of the month.<cfelseif #GetReqInfo.approved# eq 'D'>has been denied.</cfif><br /><br />
...
Additional information is required.  FMLA leaves are required after an employee has missed 11 or more consecutive days for a serious health condition including:  Their own health issue, family health issue, maternity leave, or military caregiver leave.  Please contact Julie Bitner (970-254-5120) if you have not requested FMLA paperwork.  A packet will be sent out to your home address.<br /><br />
...
<cfloop from="#Start#" to="#End#" index="a">
    ...
    <cfquery name="GetAppRequests" datasource="mesa_web">
    	SELECT	*
        FROM	LeaveReq_tblRequest
        WHERE	Approved = 'A' and yearofrequest = '2017-2018'  and EmpType = #a#
    ...
    <cfquery name="GetApproved" dbtype="query" timeout="600">
        ...
        WHERE	GetAppRequests.UserID = GetID.Username AND yearofrequest = '2017-2018'
    ...
    <cfquery name="GetPenRequests" datasource="mesa_web">
    	SELECT	*
        FROM	LeaveReq_tblRequest
        WHERE	Approved = 'P' and yearofrequest = '2017-2018' and EmpType = #a#
    </cfquery>
    ...
    <cfquery name="GetPending" dbtype="query">
        ...
        WHERE	GetPenRequests.UserID = GetID.Username AND yearofrequest = '2017-2018'
    ...
    <cfquery name="GetDenRequests" datasource="mesa_web">
    	SELECT	*
        FROM	LeaveReq_tblRequest
        WHERE	Approved = 'D' and yearofrequest = '2017-2018' and EmpType = #a#
    </cfquery>
    ...
    <cfquery name="GetDenied" dbtype="query" timeout="600">
        ...
        WHERE	GetDenRequests.UserID = GetID.Username and yearofrequest = '2017-2018'
